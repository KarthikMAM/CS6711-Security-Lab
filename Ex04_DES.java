...
public class Ex04_DES {
	...
	private static final short[] PC2 = { 
		...
		44, 49, 39, 56, 34, 53, 
		46, 42, 50, 36, 29, 32 };
	private static final short[] L_ROT = { 1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1 };
	...
	private static long mutate(long input, short[] table, long originalLength) {
		long result = 0;
		for (int i = 0; i < table.length; i++) {
			result = (result << 1) | (input >>> (originalLength - table[i])) % 2;
			// System.out.printf("%x \n", result);
		}
		return result;
	}
	...
	public Ex04_DES(long key) {
		...
		for (int i = 0; i < 16; i++) {
			c = ((c << L_ROT[i]) | (c >>> (28 - L_ROT[i]))) & GET_28B;
			d = ((d << L_ROT[i]) | (d >>> (28 - L_ROT[i]))) & GET_28B;
	...
	public static void main(String[] args) {
		...
		System.out.printf("PLAIN  TEXT : %16s \n", new String(new BigInteger(plain + "").toByteArray()));
		System.out.printf("KEY    TEXT : %16s \n", new String(new BigInteger(key + "").toByteArray()));
		System.out.printf("CIPHER TEXT : %16s \n", Long.toHexString(x.crypt(plain, true)));
		System.out.printf("PLAIN  TEXT : %16s \n", new String(new BigInteger("" + x.crypt(x.crypt(plain, true), false)).toByteArray()));
	}
